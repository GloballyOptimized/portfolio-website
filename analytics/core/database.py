...
PARQUET_PATH = pathlib.Path(__file__).parents[2] / "data" / "online_retail" / "online_retail_II.parquet"
...
def get_connection() -> duckdb.DuckDBPyConnection:
    ...
    con.execute(f"""
        ...
        WHERE Price >= 0 AND Quantity > 0   -- exclude returns & bad prices
    """)

    con.execute("""
        CREATE VIEW retail_clean AS
        SELECT * FROM retail WHERE NOT is_cancellation
    """)
    ...
    con.execute("""
        ...
        SELECT
            ...
            DATEDIFF('day', MAX(InvoiceDate),
                TIMESTAMP '2011-12-10 00:00:00')        AS recency_days
        FROM retail_clean
